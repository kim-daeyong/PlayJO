...
//    Page<Place> getPlacesByCateNo(@Param("id") int id,Pageable pageable);
    ...
    @Query("SELECT p FROM Place p " +
            "LEFT JOIN p.placeImages " +
            " order by p.id desc")
    Page<Place> getPlacesId(Pageable pageable);

    //Place의 리스트를 각 readcount로 로 정렬한다.
    @Query("SELECT p FROM Place p " +
            "LEFT JOIN p.placeImages " +
            " order by p.readCount desc")
    Page<Place> getPlacesReadcount(Pageable pageable);

    //(메인)Place의 리스트를 각 id로 로 정렬한다.
    @Query("SELECT p FROM Place p " +
            "LEFT JOIN p.placeImages " +
            " order by p.id desc")
    Page<Place> getMainPlacesId(Pageable pageable);

    //(메인)Place의 리스트를 각 readcount로 로 정렬한다.
    @Query("SELECT p FROM Place p " +
            "LEFT JOIN p.placeImages " +
            " order by p.readCount desc")
    Page<Place> getMainPlacesReadcount(Pageable pageable);
    ...
    Page<Place> getPlacesRatingAvg(Pageable pageable);
